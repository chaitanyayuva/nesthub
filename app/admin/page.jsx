...
export default function AdminOverview() {
   ...
   const [postNoticeError, setPostNoticeError] = useState(null);
   ...
   const handlePostNotice = async (data) => {
      setPostNoticeError(null);
      try {
         await postNotice(data);
         setIsPostNoticeOpen(false);
      } catch (err) {
         setPostNoticeError(err.response?.data?.message || "Failed to post notice");
      }
   };
   ...
   return (
      <div className="animate-fade-in max-w-[1400px] mx-auto px-4 sm:px-0">
            ...
            {metrics.map((stat, idx) => (
               <div key={idx} className="bg-white p-6 rounded-[32px] border border-gray-100 shadow-sm hover:shadow-xl hover:shadow-nesthub-primary/5 transition-all group duration-300">
                  <div className="flex justify-between items-start mb-6">
                     ...
                     <div className={`flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[10px] font-black uppercase tracking-tight ${stat.isPositive ? "bg-green-50 text-green-600" : "bg-red-50 text-red-600"
                        }`}>
         ...
         <AddStudentModal
            ...
            onClose={() => { setIsAddStudentOpen(false); setAddStudentError(null); }}
         ...
         <PostNoticeModal
            ...
            onClose={() => { setIsPostNoticeOpen(false); setPostNoticeError(null); }}
            onSubmit={handlePostNotice}
            error={postNoticeError}
         />
      </div>
   );
}
