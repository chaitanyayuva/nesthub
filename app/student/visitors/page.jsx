...
import { UserPlus, UserCircle, Clock, QrCode, ShieldCheck, ChevronRight, Calendar, PlusCircle, Loader2 } from "lucide-react";

export default function StudentVisitors() {
  const { pendingApprovals, historicalLogs, requestPass, loading, error } = useVisitors();
  const { user } = useAuth();
  const [isRequestModalOpen, setIsRequestModalOpen] = useState(false);
  const [requesting, setRequesting] = useState(false);
  const [requestError, setRequestError] = useState(null);
  
  // Filter for current logged in student
  const activePasses = pendingApprovals.filter(p => p.resident === user?.name);
  ...
  const handleRequest = async (data) => {
    ...
    try {
      ...
      setRequestError(err.response?.data?.message || "Failed to request pass");
    } finally {
      setRequesting(false);
    }
  };
  ...
  return (
    <div className="p-6 pb-6 max-w-lg mx-auto w-full animate-fade-in transition-all duration-300">
        ...
        <>
            ...
            activePasses.map((pass) => (
              <div key={pass.id} className="bg-white rounded-[48px] overflow-hidden shadow-2xl shadow-nesthub-primary/5 border border-gray-100 flex flex-col mb-12 animate-slide-up group hover:shadow-nesthub-primary/10 transition-all duration-500">
                ...
                <div className="px-8 pb-10 -mt-6 relative z-20 bg-white rounded-t-[40px] pt-10 flex flex-col items-center">
                   ...
                   <p className="font-mono text-[9px] text-gray-400 font-black tracking-[0.4em] uppercase mb-8">Access Token: {pass._id}</p>
          ...
          <div className="px-2">
            ...
            <div className="space-y-4">
              {historicalLogs.slice(0, 5).map((item, idx) => (
                <div 
                  key={idx} 
      ...
      <RequestVisitorPassModal
        ...
        onClose={() => { setIsRequestModalOpen(false); setRequestError(null); }}
